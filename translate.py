...
from src.config import DEFAULT_MODEL, API_ENDPOINT, LLM_PROVIDER, GEMINI_API_KEY, OPENAI_API_KEY, OPENROUTER_API_KEY, MISTRAL_API_KEY, DEEPSEEK_API_KEY, POE_API_KEY, NIM_API_KEY, OPENCODE_API_KEY, DEFAULT_SOURCE_LANGUAGE, DEFAULT_TARGET_LANGUAGE, PARALLEL_TRANSLATIONS
...
if __name__ == "__main__":
    ...
    parser.add_argument("--provider", default=LLM_PROVIDER, choices=["ollama", "gemini", "openai", "openrouter", "mistral", "deepseek", "poe", "nim", "opencode", "litellm"], help=f"LLM provider (default: {LLM_PROVIDER}). Use 'openai' for any OpenAI-compatible server. Use 'litellm' to reach 100+ providers via a provider-prefixed model name (e.g. anthropic/claude-sonnet-4-6); keys are read from each provider's native env var (OPENAI_API_KEY, ANTHROPIC_API_KEY, ...).")
    ...
    parser.add_argument("--opencode_api_key", default=OPENCODE_API_KEY, help="Opencode API key (required if using opencode provider).")
    ...
    from src.config import NIM_MODEL, MISTRAL_MODEL, DEEPSEEK_MODEL, POE_MODEL, OPENROUTER_MODEL, GEMINI_MODEL, LITELLM_MODEL, OPENCODE_MODEL
    ...
    if args.provider == "opencode" and not args.opencode_api_key:
        parser.error("--opencode_api_key is required when using opencode provider")
    if args.provider == "opencode" and args.model == DEFAULT_MODEL:
        parser.error("opencode provider requires a model. Set OPENCODE_MODEL in .env "
                     "or pass -m, e.g. -m opencode-go/deepseek-v4.1-flash")
